    ...
    <main class="flex-1 p-8">
      ...
      <section id="dashboard">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div class="bg-white p-6 rounded-lg shadow">
            ...
            <p class="text-3xl font-bold text-emerald-600"><?= $total_pesanan ?></p>
          </div>
          <div class="bg-white p-6 rounded-lg shadow">
            <h3 class="text-gray-500">Pendapatan Bulan Ini</h3>
            <p class="text-3xl font-bold text-emerald-600">Rp -</p>
          </div>
          <div class="bg-white p-6 rounded-lg shadow">
            <h3 class="text-gray-500">Pelanggan Aktif</h3>
            <p class="text-3xl font-bold text-emerald-600"><?= $pelanggan_aktif ?></p>
          </div>
        </div>
      </section>
      ...
      <section id="services" class="mt-12">
        <div class="flex justify-between items-center mb-6">
          ...
          <button onclick="toggleForm()" class="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700">+ Tambah Layanan</button>
        </div>

        <!-- Form Tambah -->
        <div id="serviceForm" class="hidden bg-white p-6 rounded-lg shadow mb-6">
          <form method="POST" class="space-y-4">
            <div><label class="block text-gray-700 mb-2">Nama Layanan</label><input type="text" name="nama_layanan" class="w-full p-2 border rounded" required></div>
            <div><label class="block text-gray-700 mb-2">Harga (Rp)</label><input type="number" name="harga" class="w-full p-2 border rounded" required></div>
            <div><label class="block text-gray-700 mb-2">Durasi Normal</label><input type="text" name="durasi_normal" class="w-full p-2 border rounded" placeholder="contoh: 3 hari" required></div>
            <div><label class="block text-gray-700 mb-2">Durasi Express</label><input type="text" name="durasi_express" class="w-full p-2 border rounded" placeholder="contoh: 1-3 hari" required></div>
            <div class="flex gap-2">
              <button type="submit" class="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700">Simpan</button>
              <button type="button" onclick="toggleForm()" class="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500">Batal</button>
            </div>
          </form>
        </div>
          ...
          <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
              <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="space-y-2">
                  <p class="text-emerald-600 font-bold">Rp <?= number_format($row['Harga'], 0, ',', '.') ?></p>
                  <div class="text-sm">
                ...
                <div class="flex gap-2 mt-4">
                  <a href="indexAdmin.php?delete=<?= $row['idLayanan'] ?>" class="text-red-600 hover:text-red-800" onclick="return confirm('Yakin ingin menghapus?')">Hapus</a>
                </div>
              </div>
            <?php endwhile; ?>
          <?php else: ?>
